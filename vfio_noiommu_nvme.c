...
printf("%d %x %d %d %x\n",device_info.argsz, device_info.flags, device_info.num_regions, device_info.num_irqs, device_info.cap_offset);
	...
	buffer = create_buffer(5*BUF_SIZE);
	if(!buffer)
		goto cleanup;
	buffer[0] = 0xEE;
	buffer[0x1000] = 0xEE;
	...
	sub_p_q1 = virtual_to_physical(sub_v_q1);
	comp_p_q1 = virtual_to_physical(comp_v_q1);
	printf("PHYSICAL %lx %lx %lx %lx %lx\n",sub_p,comp_p,prp1_p,sub_p_q1,comp_p_q1);
